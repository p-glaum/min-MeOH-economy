...
def add_generation(n, costs):
    ...
    conventionals = options.get("conventional_generation", None)

    if not conventionals:
        return
...
def add_biomass(
    ...
    if options["biogas_upgrading_cc"] and options["bio_to_gas"]:
        ...
        if len(spatial.gas.biogas) > 1 or len(spatial.co2.nodes) > 1:
            name = spatial.nodes + " biogas"
        else:
...
def add_industry(
    ...
    p_set_methanol = (
        industrial_demand.loc[(spatial.nodes, sectors_b), "methanol"]
        .groupby("node")
        .sum()
    ...
    if options["endogenous_shipping"]:
        ...
        if shipping_hydrogen_share:
            oil_efficiency = options.get(
                ...
                options.get("shipping_average_efficiency", 0.4),
            )
        ...
        if shipping_methanol_share:
            efficiency = (
                options["shipping_oil_efficiency"]
                / options["shipping_methanol_efficiency"]
            )
